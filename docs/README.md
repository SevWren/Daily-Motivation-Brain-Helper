# Daily Motivation Brain Helper — Documentation Index

> A no-code daily folder opener with motivational popup for Windows.
> Schedule tomorrow's folder in 3 clicks. No scripts. No config files. No code.

## Document Index

| File | Purpose |
|------|---------|
| [PROJECT_CHARTER.md](PROJECT_CHARTER.md) | Why this project exists |
| [VISION.md](VISION.md) | Product vision |
| [USER_STORIES.md](USER_STORIES.md) | User-centered requirements |
| [USE_CASES.md](USE_CASES.md) | Real-world scenarios |
| [NPR.md](NPR.md) | Non-Product Requirements |
| [PRD.md](PRD.md) | Product Requirements Document |
| [SSOT.md](SSOT.md) | Single Source of Truth |
| [ARCHITECTURE.md](ARCHITECTURE.md) | System design |
| [UX_SPEC.md](UX_SPEC.md) | User interaction flow |
| [FEATURE_MATRIX.md](FEATURE_MATRIX.md) | Feature inventory |
| [DATA_MODEL.md](DATA_MODEL.md) | Data structures |
| [TASK_SCHEDULER_SPEC.md](TASK_SCHEDULER_SPEC.md) | Windows scheduling logic |
| [NOTIFICATION_ENGINE_SPEC.md](NOTIFICATION_ENGINE_SPEC.md) | Popup behavior |
| [MOTIVATION_SYSTEM_SPEC.md](MOTIVATION_SYSTEM_SPEC.md) | Message management |
| [CONFIGURATION_SPEC.md](CONFIGURATION_SPEC.md) | Settings management |
| [ACCEPTANCE_CRITERIA.md](ACCEPTANCE_CRITERIA.md) | Definition of Done |
| [TEST_PLAN.md](TEST_PLAN.md) | Test strategy |
| [RISK_REGISTER.md](RISK_REGISTER.md) | Risks and mitigations |
| [ROADMAP.md](ROADMAP.md) | Release planning |
| [AGENT_WORKFLOW.md](AGENT_WORKFLOW.md) | Multi-agent workflow |
| [TRACEABILITY_MATRIX.md](TRACEABILITY_MATRIX.md) | Requirement mapping |
| [CHANGELOG.md](CHANGELOG.md) | Release history |
| [GLOSSARY.md](GLOSSARY.md) | Domain term definitions |
| [CONTRIBUTING.md](CONTRIBUTING.md) | How to contribute |
| [INSTALL.md](INSTALL.md) | End-user installation steps |
| [SECURITY.md](SECURITY.md) | Vulnerability reporting |
| [../TESTING.md](../TESTING.md) | Test coverage guide and how to run tests |
| [../Tests/README.md](../Tests/README.md) | Test suite documentation |
